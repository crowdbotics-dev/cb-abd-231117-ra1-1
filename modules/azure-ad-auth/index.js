...
const AzureADAuth = () => {
  ...
  // and further instances will also save through AzureAuth
  const [azureAuth, setAzureAuth] = useState(null);

  const [loader, setLoader] = useState(false);
  const AZURE_AUTH_OPTIONS = {
    tenant: options.AZURE_AUTH_OPTIONS_TENANT,
    clientId: options.AZURE_AUTH_OPTIONS_CLIENT_ID,
    redirectUri: options.AZURE_AUTH_OPTIONS_REDIRECT_URI
  };

  const AUTHORIZE_OPTIONS = {
    prompt: options.AUTHORIZE_OPTIONS_PROMPT,
    scope: options.AUTHORIZE_OPTIONS_SCOPE
  };
    ...
    <SafeAreaView style={options.styles.safeArea}>
      <View style={options.styles.container}>
        <View style={[options.styles.header, options.styles.backgroundColor, options.styles.alignCenter]}>
          ...
          <Text style={[options.styles.fontBold, options.styles.title, options.styles.fontSize20]}>Vodafone</Text>
        </View>
        <View style={options.styles.titleContainer}>
          <Text style={[options.styles.azureTitle, options.styles.fontBold, options.styles.fontSize20]}>Login</Text>
          <Text style={[options.styles.azureDescription, options.styles.fontSixteen, options.styles.lineHeight18]}>Welcome to Vodafone</Text>
        </View>
        ...
        <TouchableOpacity style={[options.styles.button, options.styles.commonPadding, options.styles.commonRadius, options.styles.alignCenter]} onPress={fetchUserInfo}>
          <Text style={[options.styles.btnTitle, options.styles.fontSixteen]}>Login with Vodafone</Text>
        </TouchableOpacity>
          ...
          {loader
            ...
            : <Fragment>
            {
              ...
              <Text style={[options.styles.fontSixteen, options.styles.indicationText, options.styles.lineHeight22, options.styles.textCenter]}>
                Press on &quot;Login with Vodafone&quot; button to Authenticate and to get User Information
              </Text>
            }
            <SectionList
              sections={userInfo}
